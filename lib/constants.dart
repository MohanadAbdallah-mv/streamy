import 'package:flutter/material.dart';

const Color primaryColor = Color(0xff070e13);
const Color secondryColor = Color(0xffD9DAFF);
const Color AppTitleColor = Color(0xff000000);
const Color TextFieldColor = Colors.grey;
const Color TextFieldColorFocus = Colors.grey;
const Color searchBarColor = Color(0xFF202D35);
const Color SearchBarTextFieldColor = Color(0xFF9A9EA7);
const Color AuthorColor = Color(0xff8A000000);
const Color TypingColor = Color(0x8A000000);

const Color ErrorMesseageText = Color(0xB0000000);
const Color CartItemCounterColor = Color(0xff313144);
const Color appBackGroundColor = Color(0xFF000000); //Color(0xff0E1B23);
const Color buttonColor = Color(0xff181818);
const Color mainBlueColor = Color(0xff0370D3);
const Color messageReceiverColor = Color(0xff3F454B);
const Color chatTextFieldColor = Color(0xff202D35);
const Color focusColor = Color(0xff12BFC8);
const Color smallTextColor = Color(0xff5d5d5d);
const Color titleColor = Color(0xfffcfcfc);
const Color buttonBorderColor = Color(0xff262626);

const List<Color> listColors = [
  Color(0xff12BFC8),
  Color(0xff0370D3),
  Color(0xff10A3D1),
  Color(0xff138AD2),
  Color(0xff1489D1),
];
const List<Color> buttonListColors = [
  Color(0xff0EBEC9),
  Color(0xff14A4CE),
  Color(0xff148AD3),
];
const Gradient gradient = LinearGradient(colors: listColors);
const Gradient gradientButton = LinearGradient(colors: buttonListColors);
const String messageChannel = "11";
const String messageChannelKey = "message";
const String audioCallChannel = "12";
const String audioCallChannelKey = "audioCall";
const String videoCallChannel = "13";
const String videoCallChannelKey = "videoCall";
// const Color titleColor= Color(0xfffcfcfc);
// const Color textFieldColor= Color(0xff181818);
// const Color smallTextColor= Color(0xff5d5d5d);
// const Color buttonBorderColor= Color(0xff262626);
// const Color inputColor=Color(0xffececec);
// const Color focusColor=Color(0xff12BFC8);
// const List<Color> listColors=[Color(0xff12BFC8),Color(0xff10A3D1),Color(0xff138AD2),Color(0xff1489D1)];
// const Gradient gradient= LinearGradient(colors: listColors);
